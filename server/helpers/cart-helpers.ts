...
import { NotFoundError } from 'shared/src/error-types';
...
export const validateCartRequest = (userId: string, productId?: string, quantity?: number) => {
    if (!userId) {
...
export const cleanupInvalidCartItems = async (cart: ICart): Promise<boolean> => {
